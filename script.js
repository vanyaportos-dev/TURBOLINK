...
/* ── Dashboard init (только рендер заказов, без трогания email) ── */
...
function generateServerStatuses(count) {
  ...
  statuses[redIdx] = 'red';
  const yellowCount = Math.floor(Math.random() * 5) + 3;
  let assigned = 0;
...
function randomPing() {
  return Math.floor(Math.random() * 681) + 20;
}
    ...
    return `
      <div class="server-card">
        ...
        <div class="server-status-wrap">
          <i class="fa-solid fa-circle server-status-icon ${status}"></i>
        </div>
      </div>`;
  }).join('');
}
